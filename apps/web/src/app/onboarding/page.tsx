...
export default function OnboardingPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!normalizedUrl) {
      setError("Vérifiez l’adresse de votre site, par exemple mon-entreprise.fr.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !data.site_id) {
        throw new Error(data.message ?? "Impossible de lancer l’analyse pour le moment.");
      }
    ...
    <main id="main-content" className="min-h-screen bg-[#f8f8f3] lg:grid lg:grid-cols-[0.84fr_1.16fr]">
      <aside className="relative hidden min-h-screen overflow-hidden bg-[#173b35] p-10 text-white lg:flex lg:flex-col lg:justify-between xl:p-14">
        ...
        <div className="relative">
          <Link className="inline-flex items-center gap-2.5" href="/" aria-label="SeenByAI, retour à l’accueil">
          ...
          <div className="mt-20 max-w-lg">
            <p className="text-sm font-extrabold uppercase tracking-[0.16em] text-lime-300">Votre audit commence ici</p>
            <h1 className="mt-5 text-5xl font-extrabold leading-[1.02] tracking-[-0.05em] xl:text-6xl">
              Découvrez la place que les IA accordent à votre entreprise.
            </h1>
            <p className="mt-7 text-lg leading-8 text-white/65">
              Quelques informations suffisent pour créer un jeu de requêtes adapté à votre marché local.
            </p>
          </div>
        </div>
        ...
        <div className="relative grid gap-3">
          {[
            "20 questions clients simulées",
            "ChatGPT, Claude et Perplexity comparés",
      ...
      <section className="flex min-h-screen min-w-0 flex-col">
        ...
        <div className="flex flex-1 items-center justify-center px-5 py-10 sm:px-8 lg:px-12 lg:py-14">
          <div className="w-full max-w-2xl animate-fade-in">
            <div className="mb-8 flex items-center gap-3" aria-label="Étape 1 sur 2">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-sm font-extrabold uppercase tracking-[0.14em] text-emerald-700">Profil de l’entreprise</p>
                <h2 className="mt-2 text-3xl font-extrabold tracking-[-0.04em] sm:text-4xl">Calibrons votre analyse.</h2>
                <p className="mt-2 text-base leading-7 text-muted-foreground">Renseignez votre marché principal. Vous pourrez affiner les requêtes dans le rapport.</p>
              </div>
            </div>

            <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
              <div className="space-y-2">
                <Label htmlFor="companyName" className="text-sm font-bold">Nom de l’entreprise</Label>
                <Input
                  id="companyName"
              ...
              <div className="space-y-2">
                <Label htmlFor="websiteUrl" className="text-sm font-bold">Site web principal</Label>
                <Input
                  id="websiteUrl"
                  autoComplete="url"
                  inputMode="url"
                  placeholder="maison-lenoir.fr"
                  ...
                  className="h-12 rounded-xl border-foreground/10 bg-white px-4 shadow-sm focus-visible:border-primary"
                />
                <p id="website-help" className="text-xs text-muted-foreground">Avec ou sans https://</p>
              </div>

              <div className="grid gap-5 sm:grid-cols-2">
                <div className="min-w-0 space-y-2">
                  <Label htmlFor="city" className="text-sm font-bold">Ville principale</Label>
                  <Input
                    id="city"
                ...
                <div className="min-w-0 space-y-2">
                  <Label htmlFor="industry" className="text-sm font-bold">Secteur d’activité</Label>
                  <Input
                    id="industry"
              ...
              <div className="space-y-2">
                <div className="flex items-baseline justify-between gap-3">
                  <Label htmlFor="services" className="text-sm font-bold">Services principaux</Label>
                  <span className="text-xs font-medium text-muted-foreground">Optionnel</span>
                </div>
                <Textarea
                  id="services"
                  placeholder="Cuisine sur mesure, rénovation complète, agencement…"
              ...
              <Button className="h-14 w-full rounded-xl text-base font-extrabold shadow-lg shadow-primary/12" type="submit" disabled={loading}>
                {loading ? (
                  <><Loader2 className="mr-2 size-5 animate-spin" aria-hidden="true" /> Création du rapport…</>
                ) : (
                  <>Lancer mon analyse gratuite <ArrowRight className="ml-2 size-5" aria-hidden="true" /></>
                )}
              </Button>
              ...
              <p className="flex items-center justify-center gap-2 text-center text-xs leading-5 text-muted-foreground">
                ...
                Vos informations servent uniquement à personnaliser cette simulation.
              </p>
            </form>
          </div>
        </div>
